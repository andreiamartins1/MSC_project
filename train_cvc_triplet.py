...
import os
import shutil
from utils.Dataset_cvc import TripletDataset
...
def main(logdir, name, hparams):
    ...
    ds_train = TripletDataset('data/cvc/train/', batch_size=hparams['batch_size'], target_size=target_size, loading_size=image_size, augmentations=augmentations, nb_inputs=hparams['nb_inputs'], balanced=hparams['balanced'], do_cropping=True)
    ds_val = TripletDataset('data/cvc/val/', batch_size=hparams['batch_size'], target_size=target_size, loading_size=image_size, augmentations=None, nb_inputs=hparams['nb_inputs'], shuffle=True, repeat=True, balanced=hparams['balanced'], do_cropping=True)
    ...
    model.fit(ds_train.tfdataset,
              ...
              validation_steps=10,  # len(ds_val) // hparams['batch_size'],
              callbacks=callbacks)
