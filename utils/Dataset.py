...
class Dataset(object):
    def __init__(self, image_set, batch_size=64, target_size=(256, 256), loading_size=None, nb_inputs=1, nb_classes=2, shuffle=True, repeat=True, augmentations={}, balanced=False, do_cropping=True, balance_polypids=False, selected_labels=None, oneVSall_outputs=False):
        """Init."""
    ...
    def __populate_attrs__(self):
        ...
        for row in content:
            img_path, ann_path, label, polyp_id = row.split(" ")

            if self.do_cropping == 'preloaded':
                img_path = img_path.replace('JPEGImages/1080p/', 'CROPS/256p/')

            # Label is polyp type --: label is is there polyp
        ...
        self.class_names = {0: "HP", 1: "ADN", 2: "SSP", 3: "Mucosa"}
    ...
    def combine_images_labels(self, file_path: tf.Tensor):
        ...
        for i in range(self.nb_inputs):
            ...
            if self.do_cropping == 'preloaded':
                ...
                # ann =self.load_ann)file_path
                # Calculate the size of annotation )check whether there is an annotation
