...
import random
import shutil
from utils.Dataset_quality import Dataset
import numpy as np
...
from utils.networks import MIclass, resnet, vgg16, efficientnet
...
def main(logdir, name, hparams, train_imageset='data/imagesets_1080p/{}_split_0.7.txt'.format(scope), val_imageset='data/imagesets_1080p/{}_split_0.1.txt'.format(scope)):
    ...
    ds_train = Dataset(train_imageset, batch_size=hparams['batch_size'], target_size=target_size,
                       ...
                       selected_labels=hparams['selected_labels'], nb_classes=hparams['nb_classes'], shuffle=True, top_n_frames=hparams["top_n"])
    ...
    ds_val = Dataset(val_imageset, batch_size=hparams['batch_size'], target_size=target_size, loading_size=loading_size,
                     ...
                     balanced=hparams['balanced'], do_cropping=hparams['do_cropping'], balance_polypids=hparams['balanced'],
                     selected_labels=hparams['selected_labels'], nb_classes=hparams['nb_classes'], top_n_frames=hparams["val_top_n"])
    # ds_val = Dataset(val_imageset, batch_size=hparams['batch_size'], target_size=target_size, loading_size=loading_size,
    #                  augmentations=None, nb_inputs=hparams['nb_inputs'], shuffle=True, repeat=True,
    ...
    if hparams['network'] == 'resnet':
        ...
        model = efficientnet(nb_classes=hparams['nb_classes'], image_dimensions=target_size + (3,),
                          fc_dropout=hparams['fc_dropout'], nf=hparams['nf'], multi_output=hparams['multi_output'], reg_constant=hparams['reg_constant'], nb_inputs=hparams['nb_inputs'], fold=str(k), b=hparams['b'], frozen_layers=hparams["locked_layers"])
    ...
    callbacks = []
    callbacks.append(tf.keras.callbacks.ModelCheckpoint('data/snapshots/' + name + '_best.h5', monitor='val_balanced_categorical_accuracy', mode='max', verbose=0, save_best_only=True, save_freq='epoch'))
...
if __name__ == '__main__':
    ...
    top_n_descriptor = "brisque{}".format(str(hparams["top_n"]))
    if hparams["val_top_n"]:
        top_n_descriptor += "_val_top{}".format(str(hparams["val_top_n"]))
    name = s + '_enhancedContrast_HD{scope}2024_{network}_{b}_{top_n}_regularized{reg_constant}_{target_width}x{target_height}_{nb_inputs}in_nf{nf}_bn{batch_normalization}_fcdo{fc_dropout}_balanced{balanced}_loss_{loss}_sgd'.format(scope=scope, network=hparams['network'],top_n=top_n_descriptor,b=hparams['b'], reg_constant=hparams['reg_constant'], target_width=hparams['target_width'], target_height=hparams['target_height'], nb_inputs=hparams['nb_inputs'], nf=hparams['nf'], batch_normalization=hparams['batch_normalization'], fc_dropout=hparams['fc_dropout'], conv_dropout=hparams['conv_dropout'], alpha=hparams['alpha_loss'],  resnet_layers=hparams['b'], balanced=hparams["balanced"], loss=loss_descriptor, locked_layers=hparams["locked_layers"])
    ...
    for k in range(total_k):
        ...
        # train_imageset = 'data/imagesets_1080p/{}{}_{}/{}k_train.txt'.format(total_k, split_type, scope,k)
        # val_imageset = 'data/imagesets_1080p/{}{}_{}/{}k_val.txt'.format(total_k, split_type, scope, k)

        train_imageset = 'data/imagesets_1080p/5fold_crop_brisque_all/{}{}_{}/{}k_train.txt'.format(total_k, split_type, scope,k)
        val_imageset = 'data/imagesets_1080p/5fold_crop_brisque_all/{}{}_{}/{}k_val.txt'.format(total_k, split_type, scope, k)
